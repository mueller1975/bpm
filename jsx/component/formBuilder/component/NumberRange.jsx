...
export default styled(React.forwardRef((props, ref) => {
    ...
    return (
        <div ref={ref} className={`NumberRange ${className}`}>
            <input type="number" disabled={disabled} value={minValue} onChange={minValueChanged} onBlur={updateRange} onFocus={inputFocusedHandler} />
            {
                (disabled || !inputFocused) && minValue == '' && maxValue == '' ? null : <span className="separator">～</span>
            }
            <input type="number" disabled={disabled} value={maxValue} onChange={maxValueChanged} onBlur={updateRange} onFocus={inputFocusedHandler} />
        </div>
    );
}))`
